...
Mostly focused on German North Sea, but also supports some for Baltic Sea. 
...
library(here)
here()
#EEZ_North<-st_read(file.path(here(),'tests/North_polygon.shp'))
ggplot2::ggplot()+ 
  ggplot2::geom_sf(data = EEZ_North, colour = "black", fill= '#bde0fe',alpha=0.9, lwd = 1)+
  ggplot2::coord_sf(xlim = c(3790000,4250000), ylim = c(3350000,3680000),
                    label_axes = list(top = "E", left = "N", bottom = 'E', right='N'))
# rdata
#save(EEZ_North, file = "EEZ_North.rda", version = 2)
```
...
library(here)
here()
#EEZ_Baltic<-st_read(file.path(here(),'/Baltic_EEZ_E.shp'))
ggplot2::ggplot()+ 
  ggplot2::geom_sf(data = EEZ_Baltic, colour = "black", fill= '#bde0fe',alpha=0.9, lwd = 1)
# rdata
#save(EEZ_Baltic, file = "EEZ_Baltic.rda", version = 2)
```
...
Study_areas<-st_read(file.path("C:/Users/lerma/OneDrive/Documents/03Academico/02Proyectos-Postdoc/2025/1Programming/2Packages/Shapefiles",'/strata_design_TopMarine.shp'))
```

```{r, eval=FALSE, echo=FALSE}
save(Study_areas, file="Study_areas.rda") #move to folder data
```

```{r, eval=FALSE, echo=FALSE}
usethis::use_r("Study_areas") #fill copying other information in the R folder
```
